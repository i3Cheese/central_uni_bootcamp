...
async def test_view_user_cannot_edit_board(client: AsyncClient):
    ...
    await client.post(
        "/api/v1/auth/register", json={"login": owner_login, "password": password}
    )
    await client.post(
        "/api/v1/auth/register", json={"login": viewer_login, "password": password}
    )

    owner_token = (
        await client.post(
            "/api/v1/auth/login", json={"login": owner_login, "password": password}
        )
    ).json()["token"]
    viewer_token = (
        await client.post(
            "/api/v1/auth/login", json={"login": viewer_login, "password": password}
        )
    ).json()["token"]
...
async def test_edit_user_can_edit_board(client: AsyncClient):
    ...
    await client.post(
        "/api/v1/auth/register", json={"login": editor_login, "password": password}
    )
    ...
    editor_token = (
        await client.post(
            "/api/v1/auth/login", json={"login": editor_login, "password": password}
        )
    ).json()["token"]
...
async def test_edit_user_cannot_share_board(client: AsyncClient):
    ...
    await client.post(
        "/api/v1/auth/register", json={"login": third_user_login, "password": password}
    )
...
async def test_no_access_user_cannot_get_board(client: AsyncClient):
    ...
    await client.post(
        "/api/v1/auth/register", json={"login": stranger_login, "password": password}
    )
    ...
    stranger_token = (
        await client.post(
            "/api/v1/auth/login", json={"login": stranger_login, "password": password}
        )
    ).json()["token"]
...
async def test_update_permission_from_view_to_edit(client: AsyncClient):
    ...
    await client.post(
        "/api/v1/auth/register", json={"login": user_login, "password": password}
    )
    ...
    user_token = (
        await client.post(
            "/api/v1/auth/login", json={"login": user_login, "password": password}
        )
    ).json()["token"]
