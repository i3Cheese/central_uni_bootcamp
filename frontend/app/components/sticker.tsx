...
export default function ResizableSticker({
  ...
  const handleDragStart = useCallback((e: React.MouseEvent | React.TouchEvent) => {
    ...
    const startClientX = 'touches' in e ? e.touches[0].clientX : (e as React.MouseEvent).clientX;
    ...
    const handleEnd = (endEvent: MouseEvent | TouchEvent) => {
      ...
      const clientY = 'changedTouches' in endEvent 
  ...
  return (
    ...
    >
      <div className={`${currentColor}  shadow-lg/20 p-4 border-2 border-white/50 transition-all duration-150 h-full ${
        isDragging ? 'scale-[1.02] shadow-2xl border-blue-300/50 opacity-90' : 
        isResizing ? 'scale-[1.01] border-green-300/50 shadow-xl' : 
          ...
          >
            ...
            <div className="absolute bottom-2 right-2 flex flex-col items-end justify-end opacity-0 group-hover/resize:opacity-100 group-active/resize:opacity-100 transition-opacity duration-200">
              <div className="flex gap-0.5 mb-0.5">
                <div className={`w-1.5 h-1.5 rounded-full ${isResizing ? 'bg-green-500 animate-pulse' : 'bg-blue-500'}`}></div>
                <div className={`w-1.5 h-1.5 rounded-full ${isResizing ? 'bg-green-500 animate-pulse' : 'bg-blue-500'}`}></div>
              </div>
